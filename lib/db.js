...
function getDb() {
  if (!global.__mindcareDb) {
    ...
    // Add missing columns if database was initialized in an earlier version
    const doctorColumns = db.prepare('PRAGMA table_info(doctors)').all().map((c) => c.name);
    if (!doctorColumns.includes('city')) db.exec('ALTER TABLE doctors ADD COLUMN city TEXT');
    if (!doctorColumns.includes('latitude')) db.exec('ALTER TABLE doctors ADD COLUMN latitude REAL');
    if (!doctorColumns.includes('longitude')) db.exec('ALTER TABLE doctors ADD COLUMN longitude REAL');

    const userColumns = db.prepare('PRAGMA table_info(users)').all().map((c) => c.name);
    if (!userColumns.includes('auth_provider')) db.exec("ALTER TABLE users ADD COLUMN auth_provider TEXT DEFAULT 'email'");
